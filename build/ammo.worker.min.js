...
case "gravity":setGravity(c);break;case "anchor":anchor(c);break;case "force":tmpForce.push(c);break;case "forceArray":tmpForce=c;break;case "matrix":tmpMatrix.push(c);break;case "matrixArray":tmpMatrix=c;break;case "setVehicle":setVehicle(c);break;case "contact":addContact(c)}};function preStep(){}
...
function reset(a){tmpForce=[];tmpMatrix=[];clearContact();clearJoint();clearRigidBody();clearVehicle();clearCharacter();clearSoftBody();byName={};a.full&&(clearWorld(),addWorld());setGravity();isBuffer||(Ar=new Float32Array(ArMax));self.postMessage({m:"start"})}function wipe(a){for(var b in a)a.hasOwnProperty(b)&&delete a[b]}
...
b);a.name&&(byName[a.name]=e);joints.push(e)};function stepContact(){for(var a=contactGroups.length;a--;)contactGroups[a].step()}function clearContact(){for(;0<contactGroups.length;)contactGroups.pop().clear();contactGroups=[];contacts=[]}function addContact(a){a=new Contact(a,contactGroups.length);a.valide&&(contactGroups.push(a),contacts.push(0))}
