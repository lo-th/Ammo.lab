'use strict';var Module={TOTAL_MEMORY:268435456},Ammo,start,worldscale=1,invScale=1,world=null,worldInfo=null,solver,solverSoft,collision,dispatcher,broadphase,ghostPairCallback,isSoft=!0,trans,pos,quat,posW,quatW,transW,gravity,tmpTrans,tmpPos,tmpQuat,origineTrans,tmpPos1,tmpPos2,tmpPos3,tmpPos4,tmpZero,tmpTrans1,tmpTrans2,tmpForce=[],tmpMatrix=[],bodys,solids,softs,joints,cars,heros,terrains,carsInfo,contacts,contactGroups,byName,timestep=1/60,substep=2,ddt=1,key=[0,0,0,0,0,0,0,0],tmpKey=[0,0,0,
0,0,0,0,0],isBuffer=!1,currentCar=0,Ar,aAr,ArLng,ArPos,ArMax,fixedTime=0.01667,last_step=Date.now(),timePassed=0,STATE={ACTIVE:1,ISLAND_SLEEPING:2,WANTS_DEACTIVATION:3,DISABLE_DEACTIVATION:4,DISABLE_SIMULATION:5},FLAGS={STATIC_OBJECT:1,KINEMATIC_OBJECT:2,NO_CONTACT_RESPONSE:4,CUSTOM_MATERIAL_CALLBACK:8,CHARACTER_OBJECT:16,DISABLE_VISUALIZE_OBJECT:32,DISABLE_SPU_COLLISION_PROCESSING:64},GROUP={DEFAULT:1,STATIC:2,KINEMATIC:4,DEBRIS:8,SENSORTRIGGER:16,NOCOLLISION:32,GROUP0:64,GROUP1:128,GROUP2:256,GROUP3:512,
GROUP4:1024,GROUP5:2048,GROUP6:4096,GROUP7:8192,ALL:-1};
self.onmessage=function(a){a=a.data;var b=a.m,c=a.o;a.Ar&&(Ar=a.Ar);switch(b){case "init":init(a);break;case "step":step(a);break;case "start":start(a);break;case "reset":reset(a);break;case "set":set(c);break;case "key":tmpKey=c.key;break;case "setDriveCar":currentCar=c.n;break;case "moveSoftBody":moveSoftBody(c);break;case "heroRotation":setHeroRotation(c.id,c.angle);break;case "add":add(c);break;case "addMulty":addMulty(c);break;case "vehicle":addVehicle(c);break;case "character":addCharacter(c);
break;case "terrain":terrainPostStep(c);break;case "gravity":setGravity(c);break;case "anchor":anchor(c);break;case "force":tmpForce.push(c);break;case "forceArray":tmpForce=c;break;case "matrix":tmpMatrix.push(c);break;case "matrixArray":tmpMatrix=c;break;case "setVehicle":setVehicle(c);break;case "contact":addContact(c)}};function preStep(){}
function step(a){key=a.key;updateMatrix();updateForce();terrainUpdate();world.stepSimulation(timestep,substep);drive(currentCar);move(0);stepCharacter(ArPos[0]);stepVehicle(ArPos[1]);stepRigidBody(Ar,ArPos[2]);stepSoftBody(Ar,ArPos[3]);stepConstraint(Ar,ArPos[4]);stepContact();isBuffer?self.postMessage({m:"step",Ar:Ar,contacts:contacts},[Ar.buffer]):self.postMessage({m:"step",Ar:Ar,contacts:contacts})}
function init(a){(isBuffer=a.isBuffer||!1)||(Ar=new Float32Array(ArMax));ArLng=a.settings[0];ArPos=a.settings[1];ArMax=a.settings[2];importScripts(a.blob);Ammo().then(function(b){initMath();trans=new b.btTransform;quat=new b.btQuaternion;pos=new b.btVector3;posW=new b.btVector3;quatW=new b.btQuaternion;transW=new b.btTransform;origineTrans=new b.btTransform;tmpTrans=new b.btTransform;tmpPos=new b.btVector3;tmpQuat=new b.btQuaternion;tmpPos1=new b.btVector3;tmpPos2=new b.btVector3;tmpPos3=new b.btVector3;
tmpPos4=new b.btVector3;tmpZero=new b.btVector3(0,0,0);tmpTrans1=new b.btTransform;tmpTrans2=new b.btTransform;gravity=new b.btVector3;addWorld(a.option);set(a.option);bodys=[];softs=[];joints=[];cars=[];carsInfo=[];heros=[];terrains=[];solids=[];contacts=[];contactGroups=[];byName={};self.postMessage({m:"initEngine"})})}
function set(a){a=a||{};setWorldscale(void 0!==a.worldscale?a.worldscale:1);timestep=void 0!==a.fps?1/a.fps:0.016;substep=void 0!==a.substep?a.substep:2;setGravity(a);void 0!==a.penetration&&world.getDispatchInfo().set_m_allowedCcdPenetration(a.penetration)}function setWorldscale(a){worldscale=a;invScale=1/worldscale}
function reset(a){tmpForce=[];tmpMatrix=[];clearContact();clearJoint();clearRigidBody();clearVehicle();clearTerrain();clearCharacter();clearSoftBody();byName={};a.full&&(clearWorld(),addWorld());setGravity();isBuffer||(Ar=new Float32Array(ArMax));self.postMessage({m:"start"})}function wipe(a){for(var b in a)a.hasOwnProperty(b)&&delete a[b]}function addMulty(a){for(var b=0,c=a.length;b<c;b++)add(a[b])}
...
tmpTrans.setRotation(tmpQuat));d||b||(c.setAngularVelocity(tmpZero),c.setLinearVelocity(tmpZero));d?c.getMotionState().setWorldTransform(tmpTrans):(c.setWorldTransform(tmpTrans),c.activate())}}function clearWorld(){Ammo.destroy(world);Ammo.destroy(solver);null!==solverSoft&&Ammo.destroy(solverSoft);Ammo.destroy(collision);Ammo.destroy(dispatcher);Ammo.destroy(broadphase);world=null}
function addWorld(a){a=a||{};if(null!==world)console.error("World already existe !!");else{isSoft=void 0===a.soft?!0:a.soft;solver=new Ammo.btSequentialImpulseConstraintSolver;solverSoft=isSoft?new Ammo.btDefaultSoftBodySolver:null;collision=isSoft?new Ammo.btSoftBodyRigidBodyCollisionConfiguration:new Ammo.btDefaultCollisionConfiguration;dispatcher=new Ammo.btCollisionDispatcher(collision);switch(void 0===a.broadphase?2:a.broadphase){case 1:broadphase=new Ammo.btAxisSweep3(new Ammo.btVector3(-1E3,
-1E3,-1E3),new Ammo.btVector3(1E3,1E3,1E3),4096);break;case 2:broadphase=new Ammo.btDbvtBroadphase}world=isSoft?new Ammo.btSoftRigidDynamicsWorld(dispatcher,broadphase,solver,collision,solverSoft):new Ammo.btDiscreteDynamicsWorld(dispatcher,broadphase,solver,collision)}}function setGravity(a){a=a||{};null!==world&&(gravity.fromArray(void 0!==a.gravity?a.gravity:[0,-9.81,0]),world.setGravity(gravity),isSoft&&(worldInfo=world.getWorldInfo(),worldInfo.set_m_gravity(gravity)))}
function setWater(a){isSoft&&(worldInfo=world.getWorldInfo(),worldInfo.set_water_density(a.density||0),worldInfo.set_water_offset(a.offset||0),worldInfo.set_water_normal((new Ammo.btVector3).fromArray(a.normal||[0,0,0])))};var torad=0.017453292519943295,todeg=57.29577951308232;function vectomult(a,b){return[a[0]*b,a[1]*b,a[2]*b]}
function initMath(){Ammo.btTransform.prototype.toArray=function(a,b,c){b=b||0;this.getOrigin().toArray(a,b,c);this.getRotation().toArray(a,b+3)};Ammo.btVector3.prototype.zero=function(a){this.setValue(0,0,0);return this};Ammo.btVector3.prototype.negate=function(a){this.setValue(-this.x(),-this.y(),-this.z());return this};Ammo.btVector3.prototype.add=function(a){this.setValue(this.x()+a.x(),this.y()+a.y(),this.z()+a.z());return this};Ammo.btVector3.prototype.multiplyScalar=function(a){this.setValue(this.x()*
a,this.y()*a,this.z()*a);return this};Ammo.btVector3.prototype.fromArray=function(a,b){b=b||0;this.setValue(a[b],a[b+1],a[b+2]);return this};Ammo.btVector3.prototype.toArray=function(a,b,c){c=c||1;b=b||0;a[b]=this.x()*c;a[b+1]=this.y()*c;a[b+2]=this.z()*c};Ammo.btVector3.prototype.direction=function(a){var b=a.x(),c=a.y(),d=a.z();a=a.w();var e=this.x(),f=this.y(),g=this.z(),h=a*e+c*g-d*f,k=a*f+d*e-b*g,l=a*g+b*f-c*e,e=-b*e-c*f-d*g;this.setValue(h*a+e*-b+k*-d-l*-c,k*a+e*-c+l*-b-h*-d,l*a+e*-d+h*-c-k*
...
function addJoint(a){a.body1&&(a.b1=a.body1);a.body2&&(a.b2=a.body2);var b=getByName(a.b1),c=getByName(a.b2);tmpPos1.fromArray(a.pos1||[0,0,0]).multiplyScalar(invScale);tmpPos2.fromArray(a.pos2||[0,0,0]).multiplyScalar(invScale);tmpPos3.fromArray(a.axe1||[1,0,0]);tmpPos4.fromArray(a.axe2||[1,0,0]);"joint_p2p"!==a.type&&"joint_hinge"!==a.type&&"joint"!==a.type&&(tmpTrans1.setIdentity(),tmpTrans1.setOrigin(tmpPos1),a.quatA&&(tmpQuat.fromArray(a.quatA),tmpTrans1.setRotation(tmpQuat)),tmpTrans2.setIdentity(),
tmpTrans2.setOrigin(tmpPos2),a.quatB&&(tmpQuat.fromArray(a.quatB),tmpTrans2.setRotation(tmpQuat)));var d=void 0!==a.useA?a.useA:!0,e=null,f=0;switch(a.type){case "joint_p2p":f=1;e=new Ammo.btPoint2PointConstraint(b,c,tmpPos1,tmpPos2);a.strength&&e.get_m_setting().set_m_tau(a.strength);a.damping&&e.get_m_setting().set_m_damping(a.damping);a.impulse&&e.get_m_setting().set_m_impulseClamp(a.impulse);break;case "joint_hinge":case "joint":f=2;e=new Ammo.btHingeConstraint(b,c,tmpPos1,tmpPos2,tmpPos3,tmpPos4,
d);break;case "joint_slider":f=3;e=new Ammo.btSliderConstraint(b,c,tmpTrans1,tmpTrans2,d);break;case "joint_conetwist":f=4;e=new Ammo.btConeTwistConstraint(b,c,tmpTrans1,tmpTrans2);break;case "joint_dof":f=5;e=new Ammo.btGeneric6DofConstraint(b,c,tmpTrans1,tmpTrans2,d);break;case "joint_spring_dof":f=6,e=new Ammo.btGeneric6DofSpringConstraint(b,c,tmpTrans1,tmpTrans2,d)}a.breaking&&e.setBreakingImpulseThreshold(a.breaking);a.limit&&("joint_hinge"===a.type||"joint"===a.type?e.setLimit(a.limit[0]*torad,
a.limit[1]*torad,a.limit[2]||0.9,a.limit[3]||0.3,a.limit[4]||1):"joint_conetwist"===a.type&&e.setLimit(a.limit[0]*torad,a.limit[1]*torad,a.limit[2]*torad,a.limit[3]||0.9,a.limit[4]||0.3,a.limit[5]||1));a.motor&&e.enableAngularMotor(a.motor[0],a.motor[1],a.motor[2]);a.linLower&&(tmpPos.fromArray(a.linLower),e.setLinearLowerLimit(tmpPos));a.linUpper&&(tmpPos.fromArray(a.linUpper),e.setLinearUpperLimit(tmpPos));a.angLower&&(tmpPos.fromArray(a.angLower),e.setAngularLowerLimit(tmpPos));a.angUpper&&(tmpPos.fromArray(a.angUpper),
e.setAngularUpperLimit(tmpPos));a.feedback&&e.enableFeedback(a.feedback);a.enableSpring&&e.enableSpring(a.enableSpring[0],a.enableSpring[1]);a.damping&&e.setDamping(a.damping[0],a.damping[1]);a.stiffness&&e.setStiffness(a.stiffness[0],a.stiffness[1]);a.angularOnly&&e.setAngularOnly(a.angularOnly);a.enableMotor&&e.enableMotor(a.enableMotor);a.maxMotorImpulse&&e.setMaxMotorImpulse(a.maxMotorImpulse);a.motorTarget&&e.setMotorTarget(tmpQuat.fromArray(a.motorTarget));if(a.springPosition)for(d=0;3>d;d++)0!==
a.springPosition[d]&&(e.enableSpring(d,!0),e.setStiffness(d,a.springPosition[d]));if(a.springRotation)for(d=0;3>d;d++)0!==a.springRotation[d]&&(e.enableSpring(d+3,!0),e.setStiffness(d+3,a.springRotation[d]));e.type=0;a.debug&&(e.type=f,e.bodyA=b,e.bodyB=c);world.addConstraint(e,void 0!==a.collision&&a.collision?!1:!0);a.name&&(byName[a.name]=e);joints.push(e)};function stepContact(){for(var a=contactGroups.length;a--;)contactGroups[a].step()}function clearContact(){for(;0<contactGroups.length;)contactGroups.pop().clear();contactGroups=[];contacts=[]}function addContact(a){a=new Contact(a,contactGroups.length);a.valide&&(contactGroups.push(a),contacts.push(0))}
function Contact(a,b){this.a=getByName(a.b1);this.b=void 0!==a.b2?getByName(a.b2):null;null!==this.a?(this.id=b,this.f=new Ammo.ConcreteContactResultCallback,this.f.addSingleResult=function(){contacts[b]=1},this.valide=!0):this.valide=!1}Contact.prototype={step:function(){contacts[this.id]=0;null!==this.b?world.contactPairTest(this.a,this.b,this.f):world.contactTest(this.a,this.f)},clear:function(){this.b=this.a=null;Ammo.destroy(this.f)}};function stepRigidBody(a,b){bodys.forEach(function(c,d){var e=b+8*d;a[e]=9.8*c.getLinearVelocity().length();c.getMotionState().getWorldTransform(trans);trans.toArray(a,e+1,worldscale)})}function clearRigidBody(){for(var a;0<bodys.length;)a=bodys.pop(),world.removeRigidBody(a),Ammo.destroy(a);for(;0<solids.length;)a=solids.pop(),world.removeCollisionObject(a),Ammo.destroy(a);bodys=[];solids=[]}
function addRigidBody(a,b){var c=!1;void 0!==a.density&&(a.mass=a.density);void 0!==a.bounce&&(a.restitution=a.bounce);a.kinematic&&(a.flag=2,a.state=4,c=!0);a.mass=void 0===a.mass?0:a.mass;a.size=void 0===a.size?[1,1,1]:a.size;a.pos=void 0===a.pos?[0,0,0]:a.pos;a.quat=void 0===a.quat?[0,0,0,1]:a.quat;1!==worldscale&&(a.pos=vectomult(a.pos,invScale),a.size=vectomult(a.size,invScale));var d=null;switch(a.type){case "plane":tmpPos4.fromArray(a.dir||[0,1,0]);d=new Ammo.btStaticPlaneShape(tmpPos4,0);
break;case "box":tmpPos4.setValue(0.5*a.size[0],0.5*a.size[1],0.5*a.size[2]);d=new Ammo.btBoxShape(tmpPos4);break;case "sphere":d=new Ammo.btSphereShape(a.size[0]);break;case "cylinder":tmpPos4.setValue(a.size[0],0.5*a.size[1],0.5*a.size[2]);d=new Ammo.btCylinderShape(tmpPos4);break;case "cone":d=new Ammo.btConeShape(a.size[0],0.5*a.size[1]);break;case "capsule":d=new Ammo.btCapsuleShape(a.size[0],0.5*a.size[1]);break;case "compound":d=new Ammo.btCompoundShape;break;case "mesh":for(var d=new Ammo.btTriangleMesh,
e=a.v,f=0,g=e.length;f<g;f+=9)tmpPos1.setValue(e[f+0]*a.size[0],e[f+1]*a.size[1],e[f+2]*a.size[2]),tmpPos2.setValue(e[f+3]*a.size[0],e[f+4]*a.size[1],e[f+5]*a.size[2]),tmpPos3.setValue(e[f+6]*a.size[0],e[f+7]*a.size[1],e[f+8]*a.size[2]),d.addTriangle(tmpPos1,tmpPos2,tmpPos3,!0);d=0==a.mass?new Ammo.btBvhTriangleMeshShape(d,!0,!0):new Ammo.btConvexTriangleMeshShape(d,!0);break;case "convex":for(d=new Ammo.btConvexHullShape,e=a.v,f=0,g=e.length;f<g;f+=3)e[f]*=a.size[0],e[f+1]*=a.size[1],e[f+2]*=a.size[2],
tmpPos1.fromArray(e,f),d.addPoint(tmpPos1)}void 0!==a.margin&&void 0!==d.setMargin&&d.setMargin(a.margin);if("isShape"==b)return d;if("isGhost"==b)return c=new Ammo.btGhostObject,c.setCollisionShape(d),c.setCollisionFlags(a.flag||1),c;tmpPos.fromArray(a.pos);tmpQuat.fromArray(a.quat);tmpTrans.setIdentity();tmpTrans.setOrigin(tmpPos);tmpTrans.setRotation(tmpQuat);tmpPos1.setValue(0,0,0);0!==a.mass&&d.calculateLocalInertia(a.mass,tmpPos1);e=new Ammo.btDefaultMotionState(tmpTrans);d=new Ammo.btRigidBodyConstructionInfo(a.mass,
e,d,tmpPos1);void 0!==a.friction&&d.set_m_friction(a.friction);void 0!==a.restitution&&d.set_m_restitution(a.restitution);void 0!==a.linear&&d.set_m_linearDamping(a.linear);void 0!==a.angular&&d.set_m_angularDamping(a.angular);void 0!==a.rolling&&d.set_m_rollingFriction(a.rolling);e=new Ammo.btRigidBody(d);e.isKinematic=c;a.name?byName[a.name]=e:0!==a.mass&&(byName[bodys.length]=e);0!==a.mass||c?(e.setCollisionFlags(a.flag||0),e.setActivationState(a.state||1),a.neverSleep&&e.setSleepingThresholds(0,
0),world.addRigidBody(e,a.group||1,a.mask||-1),bodys.push(e)):(e.setCollisionFlags(a.flag||1),world.addCollisionObject(e,a.group||1,a.mask||-1),solids.push(e));Ammo.destroy(d)};function stepSoftBody(a,b){var c=b;softs.forEach(function(b){b=b.get_m_nodes();for(var e=b.size(),f;e--;)f=c+3*e,b.at(e).get_m_x().toArray(a,f);c+=3*b.size()})}function moveSoftBody(a){a=softs[a.id];for(var b=a.get_m_nodes(),c=b.size();c--;);a.set_m_nodes(b)}function clearSoftBody(){for(var a;0<softs.length;)a=softs.pop(),world.removeSoftBody(a),Ammo.destroy(a);softs=[]}
