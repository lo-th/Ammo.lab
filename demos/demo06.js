...
function initDemo()
{
    ...
    for (var i=0; i!==max; ++i ){
    	...
    	py = 5+(i*6);
        ADD({type:"convex", name:"L", size:[sx, sy, sz], pos:[px, py, pz], mass:1 });
        ADD({type:"convex", name:"T", size:[sx, sy, sz], pos:[px, py+2, pz], mass:1 });
        ADD({type:"convex", name:"H", size:[sx, sy, sz], pos:[px, py+4, pz], mass:1 });
    }
}